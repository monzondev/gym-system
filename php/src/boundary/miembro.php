...
class miembro extends conector_pg
{
    ...
    private $Querys  = array(
        ...
        "proximosPagos" => "SELECT m.id_miembro, m.id_estado, m.id_tipo_membresia, m.primer_nombre, m.segundo_nombre, m.primer_apellido, m.segundo_apellido, m.usuario, m.foto, m.correo, m.genero, m.telefono, m.altura, m.peso, m.activo, m.fecha_nacimiento, m.fecha_inicio, m.inicio_membresia, m.fin_membresia FROM miembro as m WHERE (EXTRACT(DAY FROM (m.fin_membresia::timestamp)-(CURRENT_TIMESTAMP))) BETWEEN 1 AND 3 ORDER BY m.id_miembro DESC LIMIT 50",
        "proximosPagosByName" => "SELECT m.id_miembro, m.id_estado, m.id_tipo_membresia, m.primer_nombre, m.segundo_nombre, m.primer_apellido, m.segundo_apellido, m.usuario, m.foto, m.correo, m.genero, m.telefono, m.altura, m.peso, m.activo, m.fecha_nacimiento, m.fecha_inicio, m.inicio_membresia, m.fin_membresia FROM miembro as m WHERE (EXTRACT(DAY FROM (m.fin_membresia::timestamp)-(CURRENT_TIMESTAMP))) BETWEEN 1 AND 3 AND CONCAT(m.primer_nombre, ' ', m.segundo_nombre, ' ', m.primer_apellido, ' ', m.segundo_apellido, ' - ', m.usuario) ~* $1 AND m.activo=true ORDER BY m.id_miembro DESC LIMIT 5",
        "pagosEnProceso" => "SELECT m.id_miembro, m.id_estado, m.id_tipo_membresia, m.primer_nombre, m.segundo_nombre, m.primer_apellido, m.segundo_apellido, m.usuario, m.foto, m.correo, m.genero, m.telefono, m.altura, m.peso, m.activo, m.fecha_nacimiento, m.fecha_inicio, m.inicio_membresia, m.fin_membresia FROM miembro AS m WHERE ((EXTRACT(DAY FROM (CURRENT_TIMESTAMP)-(m.fin_membresia::timestamp)) >= 0) OR m.fin_membresia IS NULL) ORDER BY m.id_miembro DESC LIMIT 50",
        "pagosEnProcesoByName" => "SELECT m.id_miembro, m.id_estado, m.id_tipo_membresia, m.primer_nombre, m.segundo_nombre, m.primer_apellido, m.segundo_apellido, m.usuario, m.foto, m.correo, m.genero, m.telefono, m.altura, m.peso, m.activo, m.fecha_nacimiento, m.fecha_inicio, m.inicio_membresia, m.fin_membresia FROM miembro AS m WHERE ((EXTRACT(DAY FROM (CURRENT_TIMESTAMP)-(m.fin_membresia::timestamp)) >= 0) OR m.fin_membresia IS NULL) AND CONCAT(m.primer_nombre, ' ', m.segundo_nombre, ' ', m.primer_apellido, ' ', m.segundo_apellido, ' - ', m.usuario) ~* $1 AND m.activo=true ORDER BY m.id_miembro DESC LIMIT 5",
        ...
        "findAllActive" => "SELECT id_miembro, id_estado, id_tipo_membresia, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, usuario, foto, correo, genero, telefono, altura, peso, activo, fecha_nacimiento, fecha_inicio, inicio_membresia, fin_membresia FROM miembro  WHERE activo=true ORDER BY fecha_inicio DESC LIMIT 50",
    ...
    {   
     ...
     /*********************************************************************/
    //Metodo para obtener los miembros que estan inactivos por sus pagos atrasados
    public function getMiembrosInactivos($txt)
    {
        if($txt == ''){ # TODOS
            $query = $this->Querys['allInactive'];
            $result = pg_query($this->conexion, $query);            
        }else{ # Por nombre o usuario
            $query = $this->Querys['allInactiveByName'];
            $result = pg_query_params($this->conexion, $query, array($txt));
        }        
        if ($result) {
            $allRows = pg_fetch_all($result);
